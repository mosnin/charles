    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      ...
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Long-running goals Charles is working on
        </p>
      </header>
      ...
      {taskList.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-6 py-10 text-center">
          <p className="text-sm text-muted-foreground">
            No tasks yet. Ask Charles to work on a complex goal and it will appear here.
          </p>
        </div>
      ) : (
        <ul className="divide-y divide-border/60">
          {taskList.map((task) => {
            ...
            return (
              <li key={task.id}>
                <Link
                  href={`/s/${slug}/chat/tasks/${task.id}`}
